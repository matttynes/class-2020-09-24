...
```{r}
the_data<- inner_join(sep_old, sep_new, by= c( "name", "id", "title", "department"), 
suffix = c("_old", "_new")) %>% 
...
```{r}
top_adds<- inner_join(sep_old, sep_new, by= c( "name", "id", "title", "department"), 
suffix = c("_old", "_new")) %>% 
mutate(difference = (u_grad_new - u_grad_old)) %>% 
arrange(difference)%>% 
slice_head(n = 5)
...
```{r}
top_drops<- inner_join(sep_old, sep_new, by= c( "name", "id", "title", "department"), 
suffix = c("_old", "_new")) %>% 
mutate(difference = (u_grad_new - u_grad_old)) %>% 
arrange(desc(difference)) %>% 
slice_head(n = 5)
...
```{r}
combined <- full_join (top_adds, top_drops)

ggplot(combined, aes(reorder(name))
```
